...
- bulk tool
- 저수준 호환 도구
- 검색 결과나 `get_email` 결과에서 후속 조치 후보/확정 action metadata 생성
  또는 갱신
- 메일 서버의 읽음·이동·삭제·발송 상태는 변경하지 않고 ledger metadata만
  기록함.
- operation별 `id`를 받고 partial success 반환
- 동일 메일 identity가 이미 있으면 새 action을 만들지 않고 기존 action을
  갱신하거나 conflict를 반환함.
- schema default로 status/action type 등을 주입하지 않으며, 기존 action upsert
  시 명시하지 않은 필드는 기존 값을 보존함.

`record_mail_action_candidates`

- scope: `mail.modify`
- bulk tool
- ChatGPT 경유 신규 후보 기록 권장 도구
- 기존 메일 검색 결과의 `mailbox`와 `uid`를 기준으로 후속 조치 후보 metadata를
  MMCP 내부 ledger에 기록함.
- 필수 입력은 operation `id`, `mailbox`, `uid`만 둠.
- 선택 입력은 `uidValidity`, `uidValidityUsable`, `messageId`, `summary`,
  `reason`, `dueAt`, `deferredUntil`, `priority`, `tags`로 제한함.
- `subject`, `from`, `displaySubject`, `displayFrom`, 이메일 본문, 첨부파일,
  원본은 입력으로 받지 않음.
- 새 action 기본값은 store 내부에서 `candidate`, `review`, `none`,
  `not_needed`, `normal`로 처리함.
- 메일 서버 상태는 변경하지 않음.
...
- [x] ChatGPT 권장 신규 후보 기록 도구가 최소 입력으로 동작함.
...
- `search_mail_actions`, `get_mail_action`, `update_mail_actions`,
  `record_mail_action_location`은 ChatGPT에서 정상 동작함.
- 신규 생성 경로인 `upsert_mail_actions`는 ChatGPT/OpenAI 안전 검사 단계에서
  서버 도달 전에 차단될 수 있음.
- 원인은 synthetic record 생성 시 mailbox, messageId, subject, from,
  displaySubject, displayFrom, summary, reason, status, actionType 등이 한 번에
  들어가 “메일 생성 또는 메일 상태 변경”처럼 오해될 가능성이 큰 schema로 추정함.

대응:

- `upsert_mail_actions`는 호환용 저수준 도구로 유지함.
- ChatGPT 경유 신규 후보 기록은 `record_mail_action_candidates`를 권장함.
- `record_mail_action_candidates`는 mailbox와 UID 중심의 최소 입력만 받고, 메일
  서버 상태 변경 없음과 본문·첨부·원본 미저장을 도구 설명에 명시함.
- synthetic `uid:null` + Message-ID-only self-test는 ChatGPT 경유 검증 절차에서
  제외하고 로컬/mock 테스트 대상으로만 둠.

2026-06-11 ChatGPT 재검증 결과:

- `record_mail_action_candidates`가 기존 메일의 mailbox와 UID를 사용하는 최소
  입력으로 정상 동작함.
- tags, summary와 reason을 추가한 후보 기록도 정상 동작함.
- ChatGPT 경유 신규 후보 기록 기본 경로로 사용할 수 있음을 확인함.
- `upsert_mail_actions` 직접 호출은 ChatGPT 경유 기본 검증에서 계속 제외함.
...
- `action_type` 필드를 추가함.
- ChatGPT 경유 신규 후보 기록은 `record_mail_action_candidates`를 권장하고,
  `upsert_mail_actions`는 저수준 호환 도구로 유지함.
...
6. `upsert_mail_actions`, `record_mail_action_candidates`, `update_mail_actions`
   bulk mutation 도구를 추가함.
...
- [x] Todoist export 후보와 sync 결과 기록이 가능함.
- [x] 평문 제목·발신자가 diagnostics와 로그성 응답에 포함되지 않음.
- [x] `mail_fingerprint`가 mailbox 변경만으로 달라지지 않음.
- [x] 기존 action upsert 시 생략한 status/action type이 기존 값을 덮지 않음.
