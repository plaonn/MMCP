...
현재 수신 이메일 조회 및 상태 관리 MVP가 구현되어 있음. 네이버 메일의 편지함
목록, 이메일 검색, 개별 이메일 본문 조회, 읽음 상태 변경, 편지함 이동, 보관을
...
- `archive_email`: 단일 이메일을 네이버 보관 편지함으로 이동

메일 삭제, 휴지통 이동, 발송 기능은 제공하지 않음.
