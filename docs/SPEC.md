...
- `record_mail_action_candidates`: 기존 메일에 대한 후속 조치 후보 metadata 기록
...
- 호출 하나에 `operations`를 1~100개 지정함.
...
- 응답은 `attempted`, `succeeded`, `failed`, `results`를 반환함. `results`는
  입력 순서대로 각 작업 ID와 `succeeded` 또는 `failed` 상태를 포함하며 실패
  항목에는 오류 코드와 설명을 포함함.
- 작업은 순차적으로 실행하며 일부 작업만 성공할 수 있음. transaction과
  rollback은 지원하지 않음.
- 이동 작업은 IMAP 서버의 성공 응답 후 출발지 UID가 사라지고 목적지 UID가
  존재하는지 확인한 경우에만 성공으로 기록함.
- 응답을 받지 못한 `copy_emails` 호출을 재시도하면 중복 복사될 수 있음.
...
`record_mail_action_candidates`는 ChatGPT 경유 신규 후보 기록 권장 도구임. 필수
입력은 작업 `id`, `mailbox`, `uid`뿐이며 선택적으로 UIDVALIDITY, Message-ID,
summary/reason, 일정, priority와 tags를 받을 수 있음. 이 도구는 메일 서버의
읽음·이동·삭제·발송 상태를 변경하지 않고, 이메일 본문·첨부파일·원본을
저장하지 않음. 새 action의 기본값은 ledger store 내부에서 `candidate`,
`review`, `none`, `not_needed`, `normal`로 적용함.
