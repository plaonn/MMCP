...
- `get_emails`: 서로 다른 편지함의 이메일을 최대 20건까지 한 호출에서
  조회하고 작업별 이메일 상세 또는 안전한 실패 정보 반환
...
이메일 중복 지정을 거부함. 성공 결과는 안전한 텍스트 본문과 첨부파일
메타데이터를 포함하고, 개별 조회 실패 후에도 나머지 작업을 입력 순서대로
계속 처리함. 본문을 포함하는 응답 규모를 제한하기 위해 호출당 최대 20건을
허용함.
