...
- 응답은 `bulkId`, 도구명, 전체 상태, 상태별 처리 개수와 `results`를 반환함.
  ...
  포함함. 출발지 편지함, 출발지 UID와 작업 인자는 응답에 포함하지 않음.
- 복사·이동·휴지통·스팸 성공 항목은 후속 작업에 사용할
  `destinationMailbox`와 `destinationUid`를 포함함. 목적지 UID는 IMAP 서버가
  복사 UID mapping을 제공하지 않으면 `null`일 수 있음.
...
- 위치 변경 계열의 성공 결과는 벌크 저널에 보존하며 동일 벌크 ID 재호출과
  상태 조회에서 반환함.
